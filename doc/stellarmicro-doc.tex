...
\begin{equation}
	d \delta \eta_{(ir)} = \dfrac{2r - 1}{6} \mathcal{D}_{2}(\Lambda_0) d \ln \Lambda_0
\end{equation}
et ainsi : 
\begin{align}
  \label{eq:dPsi_dlnv}
  \frac{\d \Psi}{\d \ln v}
  &=
  1 + \dfrac{2r - 1}{12} \mathcal{D}_{2}(\Lambda_0),
  \\
  \label{eq:dPsi_dlnT}
  \frac{\d \Psi}{\d \ln T}
  &=
  \left(\frac{3}{2}+\frac{\chi}{kT}\right)
  + \dfrac{2r - 1}{4} \mathcal{D}_{2}(\Lambda_0).
\end{align}
...
\begin{equation}
	\begin{split}
		...
		&= - \d_{\ln v} F_0 + \sum_{ir} x_i y_i^r + \dfrac{1}{12} \mathcal{D}_{2}(\Lambda_0) \sum_{ir} x_i (2r-1) y_i^r.
	\end{split}
\end{equation}
...
Finalement, 
\begin{equation}
	\Omega(v, T, \vec{X}) = 1 + \langle r \rangle + \dfrac{\langle r^2 \rangle}{12} \mathcal{D}_{2}(\Lambda_0).
\end{equation}
...
\begin{equation}
	\begin{split}
		...
		&= - \d_{\ln T} F_0 + \dfrac{3}{2} \langle r \rangle + \dfrac{\langle \chi \rangle}{kT} + \dfrac{\langle r^2 \rangle}{4} \mathcal{D}_{2}(\Lambda_0),
	\end{split}
\end{equation}
fournissant la nouvelle contrainte sur la jauge $\d_{\ln T} F_0 = - 3/2$.
...
\]
En effet, puisque $Ts = \varepsilon - f$ :
\begin{equation}
	\Sigma = \dfrac{3}{2} (1 + \langle r \rangle) + \dfrac{\langle \chi \rangle}{kT} + \dfrac{\langle r^2 \rangle}{4} \mathcal{D}_{2}(\Lambda_0) - F_0 + x_{\text H} y_{\text H} - \sum_{ir} x_i \ln (1 - y_i^r)
\end{equation}
...
Notons qu'à partir du moment où les deux potentiels de transition ont été définis, il est possible de retrouver les potentiels thermodynamiques restants à partir des relations \eqref{eq:legendre_potential_eps}-\eqref{eq:legendre_potential_h} : 
\begin{equation}
	E = F_{\star} + \Sigma, 
	\qquad
	G = F_{\star} + \Omega,
	\qquad
	H = F_{\star} + \Sigma + \Omega
\end{equation}
